...
Your own files contribute to `+red` only after you [start self-reviewing](reviews.md#user-roles), including with a draft mark. The `+needs:me` filter continues to follow who the review is waiting on. Author files never match `+deferred`.
