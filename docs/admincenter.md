...
An array of objects that looks like `{path: 'full/path/to/file', group: 'Some Group', revisions: [key: 'r1', reviewed: true]}`.  (You can augment the `review.files` structure with additional properties and return the whole thing here.)
  ...
  - If you just want author marks to count towards completion, you can use the following minimal snippet.  Note that this will not affect how pending reviewers are computed:

    ```js
    const files = review.files.concat(review.systemFiles);

    for (const file of files) {
      for (const revision of file.revisions) {
        revision.reviewed = revision.reviewers.length > 0;
      }
    }
...
```
`data.pullRequest.requestedTeams` lists the bare slugs of teams with an outstanding GitHub review request, without an organization prefix.  The slugs are sorted, and the array is empty when no teams are requested.  Adding or removing teams triggers a notification even when the other tracked fields are unchanged; reordering the same teams does not.  GitHub generates lowercase slugs, so changing only the capitalization of a team's display name does not change this field.

`data.review.stage` contains the [review stage](#stage) set by the completion condition and is omitted when no stage is set.
