...
{:important}
Both the URL template and the header will be available to all users with read permissions on this
repo.  Be careful what capabilities you expose here, and remember that users can make a copy of any
tokens to take with them when they leave!

There's a button to let you easily set the report source to [Codecov](https://codecov.io), a popular code coverage report aggregation service.  If you're using a self-hosted instance of Codecov Enterprise then you'll need to set the URL to something like this instead:  `https://LOCAL_CODECOV_HOSTNAME/api/ghe/{{owner}}/{{repo}}/commits/{{commitSha}}?src=extension`, with `LOCAL_CODECOV_HOSTNAME` replaced by the name of the host where you're running Codecov.
