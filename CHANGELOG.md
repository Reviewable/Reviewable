...
- Adm: migrate to an integrated SaaS/Enterprise changelog.
...
- Adm(enterprise): **WARNING** This version includes a fix to the task queuing system that is likely to result in a burst of task processing activity on first launch.  This may be high enough to temporarily overload Firebase, so we recommend that you schedule the upgrade for off-hours.
...
- Adm(enterprise): **WARNING** this release breaks sign-in on installations that configured encryption.
...
- Adm(enterprise): **WARNING** this release only works with GHE 3.11+.
...
- Adm(enterprise): **WARNING** a bug in recent older versions breaks rollbacks, so the minimum version here is set to prevent them.  If you really need to roll back please get in touch -- we know how to craft a workaround given a specific version to roll back to.
...
- Adm(enterprise): **WARNING** this release is broken if you're using encryption.
...
- Adm(enterprise): **WARNING** access to the Repositories page by the license admin is broken in this release.
...
- Adm(enterprise): **WARNING** the `vm2` executor is broken in this release.
...
- Fix(enterprise): **HOTFIX** for GHE 3.0: sync large PRs. GHE 3.0 made a subtle change to one of their APIs that made Reviewable fail when syncing PRs with very large diffs.
...
- Fix: **HOTFIX** make custom review completion conditions work again.  These got broken back in 2630.4363 for both the Sandcastle executor (completely) and for AWS Lambda (for new repos only).
...
- Adm(enterprise): **WARNING** custom review completion condition may not execute correctly in this release.
...
- Adm(enterprise): This version is broken on some older GHE versions, it's strongly recommended that you skip to the next one (2148.3676).
...
- Adm(enterprise): **KNOWN ISSUE** This version breaks badly in environments running in `REVIEWABLE_PRIVATE_MODE`.  You almost certainly want to skip over it.
...
- New(enterprise): initial public release.
