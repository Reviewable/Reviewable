...
- <kbd>Fix:</kbd> continue loading completion lists when the browser blocks IndexedDB access. <!-- client -->
...
- <kbd>Adm<i>(enterprise)</i>:</kbd> prevent users from obtaining a long-term access token by issuing a guest seat upgrade request with a bogus organization.  This was merely a license provisioning bug with no security implications. <!-- server 4968 -->
- <kbd>Adm:</kbd> correctly detect API quota errors on GitHub REST requests.  (Regression introduced in v4814.7709.) <!-- server 4968 -->
- <kbd>Fix:</kbd> don't accidentally drop sync tasks when GitHub responds with `Retry-After` due to a burst quota overrun. <!-- server 4968 -->
- <kbd>Fix:</kbd> preserve merge-queue status checks during routine syncs so custom completion conditions evaluate the correct check results. <!-- server 4968 -->
- <kbd>Fix:</kbd> keep reviews from getting stuck on "Mapping renamed files" for non-push viewers. <!-- client 7908 -->
- <kbd>Fix:</kbd> show binary size totals in the file matrix. <!-- client 7908 -->
- <kbd>Fix:</kbd> show byte sizes and size changes for files marked binary or -text in .gitattributes. <!-- client 7908 -->
- <kbd>Fix:</kbd> don't jump to an unrelated file when marking a file reviewed and advancing. <!-- client 7908 -->
- <kbd>Upd:</kbd> Truncate file names and avatars in narrow sidebar file matrix. <!-- client 7908 -->
...
- <kbd>Adm:</kbd> reduce the amount of work done in reaction to a push to a base branch when there are many stale pull requests targeting it. <!-- server 4950 -->
...
- <kbd>Adm:</kbd> improve logging of available memory and other system details on startup. <!-- server 4942 -->
