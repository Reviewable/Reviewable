...
// the ones you can set up on Pull Approve.  Note that if a file is covered by multiple conditions
// then they will *all* have to pass for the file to be considered as reviewed.  You can use the
// functions to establish a series of stages and gates between them like so:
...
function applyWorkflow(...steps) {
  ...
  const slugs = _.uniq(requiredTeamSlugs);
  const description =
    (stage || '') + (stage && slugs.length ? ': ' : '') +
    (slugs.length ? `needs approval${plural(slugs)} from ${slugs.join(', ')}` : '');
...
function approved({
  ...
  if (!changesRequested) numApproved += _(relevantSanctions).filter({state: 'approved'}).size();
...
function reviewed({
  reviews = 1, team, request = true, unrequest = true, author, coauthor, paths, notPaths, label,
  ...
  for (const file of selectedFiles) {
    ...
    for (const rev of file.revisions) {
      ...
      satisfied = numReviewers >= reviews;
      if (rev.reviewed !== false) rev.reviewed = satisfied;
    ...
    if (team) {
      if (satisfied) {
        if (unrequest) _.remove(requestedTeams, {slug: team});
      } else {
        requiredTeamSlugs.push(team);
...
function includesTeam(teams, team) {
  if (!/\//.test(team)) team = `${review.pullRequest.repository.owner}/${team}`;
  team = _.toLower(team);
  return _(teams).map(_.toLower).includes(team);
}
