...
return applyWorkflow(
  ...
  () => [  // all of the following must be satisfied, and are evaluated concurrently
    reviewed({reviews: 2, team: {'graphql-admin': 2, 'graphql-dev': 1}, paths: 'backend/*.graphql', coauthor: true}),
 ...
 * @param {*} params
 * - `approvals`: the approval score required (defaults to 1); there also can't be any changes
...
function approved({
  ...
  const satisfied = approvedScore >= approvals;
  if (team) {
    if (!satisfied) {
      requiredTeams.push(..._.keys(team));
      if (request && involvedScore < approvals) requestedTeams.push(..._.keys(team));
      if (involvedScore) {
 ...
 * @param {*} params
 * - `reviews`: the review score required (defaults to 1)
 ...
 */
function reviewed({
  reviews = 1, team, request = true, unrequest = true, author, coauthors, paths, notPaths, label,
  ...
  const implicitReviewScore = team ?
