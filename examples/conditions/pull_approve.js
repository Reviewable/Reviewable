...
return applyWorkflow(
  ...
  () => [  // all of the following must be satisfied, and are evaluated concurrently
    reviewed({targetScore: 2, team: {'graphql-admin': 2, 'graphql-dev': 1}, paths: 'backend/*.graphql', coauthors: true}),
 ...
 *   (optional)
 * - `notLabel`: a label that, if present on the pull request, prevents this requirement from being
 *   applied (optional)
 */
function approved({
  targetScore = 1, team, request = true, author, coauthors, paths, notPaths, label, notLabel
} = {}) {
  const pr = review.pullRequest;
  ...
  if (label && !hasLabel(label)) return true;
  if (notLabel && hasLabel(notLabel)) return true;
  ...
  if (team) {
    ...
    for (const file of selectedFiles) {
      ...
      const scope = _.keys(team).join(', ');
      file.designatedReviewers.push(..._.map(team, (unused, key) => ({team: key, scope})));
      if (satisfied) {
 ...
 */
function reviewed({
  targetScore = 1, team, request = true, unrequest = true, author, coauthors, paths, notPaths,
  label, notLabel, omitBaseChanges = true
} = {}) {
  const pr = review.pullRequest;
  team = normalizeTeam(team);
  if (team && author === undefined) author = true;
  ...
  for (const file of selectedFiles) {
    ...
    if (team) {
      file.designatedReviewers.push(
        ..._.map(team, (unused, key) => ({team: key, scope, omitBaseChanges})));
      if (satisfied) {
        file.designatedReviewers.push(
          ..._.map(team, (unused, key) => ({builtin: 'fulfilled', scope})));
...
/**
 * Check if a label is present on the pull request.
 */
function hasLabel(label) {
  return _.some(review.labels, reviewLabel => _.toLower(reviewLabel) === _.toLower(label));
}
