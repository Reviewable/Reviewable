...
//      help fulfill any scopes that are now unfulfilled, or a sole specifically designated user.
...
function applyDesignatedReviewers(files) {
  ...
  _.forEach(files, file => {
    ...
    // appeared.  A lone specifically designated user is also woken directly; otherwise, if nobody
    // matching a scope has reviewed the file yet, leave it unclaimed for the normal fallback.
    _(designationsByScope).keys().difference(fulfilledScopes).forEach(scope => {
      ...
      } else if (subjects.length === 1 && subjects[0].username) {
        fileBlockers.push({username: subjects[0].username});
      } else {
        hasUnclaimedFiles = true;
      }
    });
