...
Set the application name, homepage URL, and application description to taste (but preferably not just plain "Reviewable" to avoid confusion).  You can easily update these later so don't sweat it.  Set the authorization callback URL to `<REVIEWABLE_HOST_URL>/auth/callback`.

If you have an Optional features > Token expiration setting for your app then make sure to opt out of this feature.

Take note of the Client ID and Client Secret at the top of the application's dashboard as you'll need to provide these to Reviewable (below).
...
* `REVIEWABLE_WEBHOOK_BLACKLIST`: (optional but recommended) Restrict the destinations that completion condition webhooks are allowed to POST to, so that anyone able to define a completion condition (via the admin UI, or by landing a `.reviewable/completion.js` on a connected repository's default branch) can't point one at services on your internal network or at cloud metadata endpoints.  Set to a comma-separated list of IPv4/IPv6 addresses and CIDR blocks to refuse, or to `local` (recommended) to block all loopback, private, link-local (including the cloud metadata endpoint), and other non-routable and special-use ranges.  Destinations are matched against the address each request actually connects to, so host names that resolve to a blocked address (and any redirects) are refused as well.  Unset by default, applying no filtering, since some installations legitimately send webhooks to internal hosts.
* `REVIEWABLE_COVERAGE_URL_WHITELIST`: A comma-separated list of base URLs (each `scheme://host[:port]/path`) that the [code coverage](https://docs.reviewable.io/admincenter#code-coverage) proxy is allowed to fetch reports from.  A repository's configured coverage `url` is only fetched if the resulting URL falls under one of these base URLs at a path boundary.  For example, `https://codecov.example.com/api/ghe` permits `https://codecov.example.com/api/ghe/owner/repo/commits/...`.  The request carries the coverage `headers` (which may include authorization tokens), so that a repo-controlled `url` can't be used to direct those credentials — or the server's outbound requests — at arbitrary internal hosts.  Codecov's public API (`codecov.io` and `api.codecov.io`) is always allowed.  Set this variable if you fetch coverage from a self-hosted or third-party server.
...
* `REVIEWABLE_LOGGING_FORMAT`: Set to `gcp` to write structured JSON logs to the console in Google Cloud Logging's format.  Plain log messages will be stored in the `message` field, and logs emitted while handling a queue event will include an operation ID of the form `queues/<queue>/<event>#attempt-<number>` so they can be grouped and filtered by task.  If unset, console logs will use the default plain-text format.
