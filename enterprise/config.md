...
* `REVIEWABLE_GITHUB_CERT_FILE`: The absolute path to your GitHub server's TLS cert file.  Useful if your GitHub installation uses a self-signed (or otherwise non-validating) certificate, as otherwise Reviewable will refuse to connect to the API.  Note that all client machines will need this cert too to make your browsers happy but presumably you've already done this.
