...
- Upd: updated server build process.  There should be no impact on vanilla deployments.  If you customized things please note that the new image entrypoint is `node dist/main.js` and the code is now distributed minified (and without source maps) so stack traces will look different.
