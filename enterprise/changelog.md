...
- Fix: reduce Docker image size back <90MB.  The previous release accidentally bloated it a bit.
...
- New: display a revision cell with both an `action` and a `prior action` when rebased revisions are matched using heuristics. See the [docs](https://docs.reviewable.io/files.html#revision-cells) for details.
